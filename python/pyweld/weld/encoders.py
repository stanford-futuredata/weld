...
class NumpyArrayEncoder(WeldObjectEncoder):
    ...
    def encode(self, obj):
        ...
        return WeldVec(elem_type).ctype_class(ptr=obj.ctypes.data_as(POINTER(elem_type.ctype_class)), size=ctypes.c_int64(obj.size))
...
class NumpyArrayDecoder(WeldObjectDecoder):
    ...
    def decode(self, obj, restype):
        ...
        if restype == WeldVec(WeldLong()) or restype == WeldVec(WeldDouble()):
            ArrayType = ctypes.c_double*size
        elif restype == WeldVec(WeldInt()) or restype == WeldVec(WeldFloat()):
